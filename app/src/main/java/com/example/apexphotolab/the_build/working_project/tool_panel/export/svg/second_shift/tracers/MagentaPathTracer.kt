...
object MagentaPathTracer {
    fun trace(homeCandidates: List<Point>, vram: ByteBuffer, width: Int, pixels: IntArray, remainingPixels: MutableSet<Point>? = null): List<List<Point>> {
        ...
        for (home in homeCandidates) {
            ...
            while (true) {
                ...
                if (nextPoint == null) nextPoint = findRecoveryPoint(currentPoint, myRemaining, vram, width)
                if (nextPoint != null) {
    ...
    private fun findNextStep(current: Point, previous: Point?, remaining: Set<Point>, home: Point, pathSize: Int, vram: ByteBuffer, width: Int): Point? {
        ...
        for (i in 0 until 8) {
            ...
            if (remaining.contains(neighborPoint) && getBit(vram, ny * width + nx)) return neighborPoint
        }
        return null
    }
    private fun findRecoveryPoint(current: Point, remaining: Set<Point>, vram: ByteBuffer, width: Int): Point? {
        for (r in 1..3) {
            for (dy in -r..r) {
                for (dx in -r..r) {
