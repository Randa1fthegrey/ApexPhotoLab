...
/**
 * Job: VPS Job 3 - Census.
 * Responsibility: Analyzing a path region via flood fill to produce a statistical CensusReport.
 */
object VPS_job3 {
    ...
    data class CensusData(
        ...
        var reportResult: CensusReport? = null
    )

    suspend fun execute(workerId: Int, data: Any?) {
        val cData = data as? CensusData ?: return
        val path = cData.path
        val quantizedImage = cData.quantizedImage
        val vramSlot = cData.vramSlot

        if (path.isEmpty()) {
            cData.reportResult = CensusReport(Color.TRANSPARENT, Color.TRANSPARENT, 0, 0f, 0)
            return
        }
        ...
        if (boxWidth == 0 || boxHeight == 0) {
            cData.reportResult = CensusReport(identityColor, identityColor, 0, 0f, 0)
            return
        }
        ...
        cData.reportResult = CensusReport(
        ...
        while (queue.isNotEmpty()) {
            ...
            for (neighbor in neighbors) {
                if (neighbor.x in 0 until width && neighbor.y in 0 until height) {
                    ...
                    // --- COLOR WALL CHECK ---
                    if (ColorGroupSorter.getGroupIndexForPixel(localPixels[pixelIndex]) != targetGroup) continue
