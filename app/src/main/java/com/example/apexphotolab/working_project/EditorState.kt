...
class EditorState(
    ...
    fun startExport(context: Context, directoryUri: Uri, scope: CoroutineScope) {
        ...
        exportJob = scope.launch {
            try {
                com.example.apexphotolab.working_project.tool_panel.export.data.ExportJobManager.runExportJob(
                    context = context,
                ...
                android.widget.Toast.makeText(context, "Export Finished", android.widget.Toast.LENGTH_SHORT).show()
            } catch (e: Exception) {
    ...
    fun addLayerFromUri(context: Context, projectUri: Uri, imageUri: Uri, title: String, scope: CoroutineScope) {
        scope.launch {
            val newLayer = com.example.apexphotolab.working_project.tool_panel.layers.NewLayerManager.addNewLayer(context, projectUri, imageUri, title)
            if (newLayer != null) {
                addLayer(newLayer)
            } else {
                android.widget.Toast.makeText(context, "Failed to add layer", android.widget.Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    fun saveSnapshot(context: Context, projectUri: Uri, note: String, scope: CoroutineScope, drawerState: DrawerState) {
        scope.launch {
            com.example.apexphotolab.working_project.tool_panel.save.ProjectSaveManager.saveProject(context, projectUri, layers.toList(), note)
            updateSaveTime()
    ...
    fun rollbackProject(context: Context, projectUri: Uri, snapshot: DocumentFile, scope: CoroutineScope, drawerState: DrawerState) {
        scope.launch {
            ...
            if (success) {
                closeAllPanels(scope, drawerState)
                android.widget.Toast.makeText(context, "Project Restored", android.widget.Toast.LENGTH_SHORT).show()
            } else {
                android.widget.Toast.makeText(context, "Rollback Failed", android.widget.Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    fun quickSaveProject(context: Context, projectUri: Uri, scope: CoroutineScope) {
        scope.launch {
            ...
            if (success) {
                android.widget.Toast.makeText(context, "Saved project!", android.widget.Toast.LENGTH_SHORT).show()
            } else {
    ...
    fun handleMoveGesture(pan: androidx.compose.ui.geometry.Offset, zoom: Float, rotation: Float) {
