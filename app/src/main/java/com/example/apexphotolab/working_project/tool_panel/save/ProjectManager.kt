...
import com.example.apexphotolab.working_project.tool_panel.layers.LayerSerializer
...
object ProjectManager {
    ...
    private fun getProjectsDir(context: Context): DocumentFile? {
        val customDirUri = SettingsPersistence.getProjectDir(context)
    ...
    suspend fun createProject(context: Context, projectName: String, baseImageBitmap: Bitmap): DocumentFile? =
        withContext(Dispatchers.IO) {
            try {
                ...
                val baseImageFile = imagesDir.createFile("image/png", "testing/base_image.png")!!
     ...
     * Also persists any live edits from BitmapRegistry to the filesystem.
    ...
    suspend fun saveProject(context: Context, projectDir: DocumentFile, layers: List<Layer>, title: String) =
        withContext(Dispatchers.IO) {
            try {
                // 1. Sync any live edits from BitmapRegistry to the filesystem
                ...
                val updatedLayers = layers.map { layer ->
                    val workingBitmap = BitmapRegistry.getWorkingBitmap(layer.id)
                    if (workingBitmap != null) {
                        // We have live edits, so we need to overwrite the image on disk
                        val imageFile = imagesDir.findFile(layer.imageUri.lastPathSegment ?: "${layer.id}.png")
                            ?: imagesDir.createFile("image/png", "${layer.id}.png")!!
                        
                        context.contentResolver.openOutputStream(imageFile.uri)?.use { out ->
                            workingBitmap.compress(Bitmap.CompressFormat.PNG, 100, out)
                        }
                        // Update layer with the potential new URI if it was created
                        layer.copy(imageUri = imageFile.uri)
                    } else {
                        layer
                    }
                }
                ...
                updatedLayers.forEach { layer -> layersJson.put(LayerSerializer.toJson(layer)) }
                ...
                // 2. Write the snapshot to the project-specific history directory
