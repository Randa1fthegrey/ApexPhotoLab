...
/**
 * A manual Adobe PSD (Photoshop) file writer.
 * Implements the layered structure (8-bit RGBA) without third-party libraries.
 * Follows the Adobe Photoshop File Formats Specification.
 */
object PsdLayeredSaver {
    ...
    suspend fun saveLayeredPsd(
        ...
        val flattened = ImageFlattener.flattenLayers(context, layers, false, canvasWidth, canvasHeight)
        if (flattened != null) {
    ...
    private suspend fun renderLayerToBitmap(context: Context, layer: Layer, canvasWidth: Int, canvasHeight: Int): RenderedLayerInfo {
        ...
        val srcRect = android.graphics.RectF(0f, 0f, original.width.toFloat(), original.height.toFloat())
