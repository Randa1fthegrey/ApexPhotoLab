...
/**
 * A manual GIMP XCF v1 (32-bit) writer.
 * Implements a layered XCF structure using Tiled RGBA data.
 * Optimized with Hybrid Streaming to handle high-res (8k+) images without crashing.
 */
object XcfSaver {
    ...
    suspend fun saveLayeredXcf(
        ...
        val layerInfos = layers.map { layer ->
            val bitmap = loadBitmap(context, layer.imageUri) ?: Bitmap.createBitmap(1, 1, Bitmap.Config.ARGB_8888)
