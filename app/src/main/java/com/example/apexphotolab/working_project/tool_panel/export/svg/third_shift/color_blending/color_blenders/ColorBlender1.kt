package com.example.apexphotolab.working_project.tool_panel.export.svg.third_shift.color_blending.color_blenders
...
object ColorBlender1 : ColorBlender {
    ...
    override fun blend(
        ...
        // 1. Determine the Bounding Box and Axis
        ...
        // 2. Setup the Ladder (13 points)
        // 13 points = 12 intervals. 
        ...
        for (i in 0 until sampleCount) {
            ...
            // Sample along the center of the cross-axis
        ...
        val x1 = if (isHorizontal) "0%" else "0%"
        val y1 = if (isHorizontal) "0%" else "0%"
        val x2 = if (isHorizontal) "100%" else "0%"
        val y2 = if (isHorizontal) "0%" else "100%"

        val pathData = PathDataGenerator.generate(path)

        // Added stroke-width="0.5" and stroke with the first color to reduce gaps between shapes
        val firstColor = String.format("#%06X", 0xFFFFFF and sourceImage.getPixel(
            bounds.left.coerceIn(0, sourceImage.width - 1),
            bounds.top.coerceIn(0, sourceImage.height - 1)
        ))
        ...
        return buildString {
            ...
            append("<path d=\"$pathData\" fill=\"url(#$gradId)\" stroke=\"$firstColor\" stroke-width=\"0.5\" stroke-linejoin=\"round\" />")
        }
    }
