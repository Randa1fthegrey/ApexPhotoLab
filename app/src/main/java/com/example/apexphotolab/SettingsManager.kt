...
object SettingsManager {
    ...
    fun getCustomProjectDir(context: Context): Uri? {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val uriString = prefs.getString(KEY_PROJECT_DIR_URI, null)
        return uriString?.let { Uri.parse(it) }
    }

    fun setCustomProjectDir(context: Context, uri: Uri) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit()
    ...
    fun clearCustomProjectDir(context: Context) {
        val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit()
        prefs.remove(KEY_PROJECT_DIR_URI)
        prefs.apply()
    }
    
    fun isProjectDirValid(context: Context): Boolean {
        val uri = getCustomProjectDir(context) ?: return false
        val docFile = DocumentFile.fromTreeUri(context, uri)
        return docFile != null && docFile.canRead()
    }
